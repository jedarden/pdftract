#!/usr/bin/env python3
"""
Verify test function signatures in the pdftract codebase - FIXED VERSION.
...
def extract_tests_and_attrs(content: str, filepath: str) -> List[Tuple[str, List[str], int, str]]:
    """
    ...
    for i, line in enumerate(lines, 1):
        ...
        if match:
            ...
            results.append((func_name, attrs, i, signature))

    return results


def extract_false_positives(content: str, filepath: str) -> List[Tuple[str, List[str], int, str]]:
    """
            ...
            if func_line:
                ...
                if match:
                    ...
                    if not func_name.startswith('test_'):
                        results.append((func_name, func_attrs, func_line_num, func_line))
...
def has_test_attribute(attrs: List[str]) -> bool:
    """Check if any of the attributes is a test attribute."""
    return any(any(test_attr in attr for test_attr in TEST_ATTRS) for attr in attrs)
...
def main():
    ...
    for filepath in rust_files:
        ...
        for func_name, attrs, line_num, signature in tests:
            ...
            has_test_attr = has_test_attribute(attrs)
    ...
    print(f"  Valid tests (with #[test]): {stats['valid_tests']}")
    ...
    if issues:
        ...
        if missing_test:
            ...
            for issue in missing_test[:10]:
                ...
                attr_str = issue['attrs'][-1] if issue['attrs'] else 'none'
                print(f"    Last attribute: {attr_str}")
