//! Python extract_markdown() entry point using PyO3.
//!
//! This module provides the extract_markdown() function that returns Markdown
...
use pdftract_core::sdk::extract_markdown;
...
/// document structure with headers, links, and other formatting. It uses
/// the SDK's extract_markdown implementation which builds Markdown via
/// page_to_markdown_with_links (blocks/spans/links, per-page).
///
...
/// # Returns
///
/// A Python string containing the extracted Markdown. The output includes
/// Markdown headings (`#`), links (`[text](url)`), and other structural elements.
///
/// # Examples
///
/// ```python
...
///
/// # With page range
/// markdown = pdftract.extract_markdown("doc.pdf", pages="1-5")
///
/// # With invisible text included
/// markdown = pdftract.extract_markdown("doc.pdf", include_invisible=True)
///
/// # With password for encrypted PDF
/// markdown = pdftract.extract_markdown("encrypted.pdf", password="secret123")
/// ```
///
/// # Errors
///
...
pub fn extract_markdown_fn(py: Python<'_>, path: &str, kwargs: Option<&PyDict>) -> PyResult<String> {
    ...
    let markdown = py
        .allow_threads(|| extract_markdown(pdf_path, &opts))
        .map_err(|e| {
