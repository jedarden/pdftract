...
fn test_filter_array_debug() {
    let encoded = [
        0x3c, 0x7e, 0x6f, 0x31, 0x37, 0x2d, 0x4a, 0x61, 
        0x6b, 0x27, 0x41, 0x71, 0x63, 0x53, 0x2a, 0x46, 
        0x34, 0x3b, 0x24, 0x36, 0x6b, 0x7e, 0x3e,
    ];
    ...
    if let Ok(a85_decoded) = result1 {
        ...
        match &result2 {
            Ok(bytes) => println!("After Flate ({:?} bytes): {:?}", bytes.len(), String::from_utf8_lossy(bytes)),
