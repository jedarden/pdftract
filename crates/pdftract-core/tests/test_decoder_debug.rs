...
fn test_decoder_debug() {
    ...
    let result = LZWDecoder.decode(
        ...
        Some(&params_obj),
        &mut counter,
        pdftract_core::parser::stream::DEFAULT_MAX_DECOMPRESS_BYTES,
    );
    match &result {
    ...
    let a85_input =
        std::fs::read("tests/stream_decoder/fixtures/filter_array_a85_then_flate.bin").unwrap();
    println!(
        "ASCII85 input (first 50 bytes): {:02x?}",
        &a85_input[..a85_input.len().min(50)]
    );

    let mut counter = 0u64;
    let result = ASCII85Decoder.decode(
        &a85_input,
        None,
        &mut counter,
        pdftract_core::parser::stream::DEFAULT_MAX_DECOMPRESS_BYTES,
    ...
    match &result {
        Ok(data) => {
            println!(
                "ASCII85 decoded (first 50 bytes): {:02x?}",
                &data[..data.len().min(50)]
            );
            println!(
                "ASCII85 decoded as string: {:?}",
    ...
    let flate_input =
        std::fs::read("tests/stream_decoder/fixtures/flate_png_pred15_all_six.bin").unwrap();
    println!(
        "Flate input (first 50 bytes): {:02x?}",
        &flate_input[..flate_input.len().min(50)]
    );
    ...
    match &result {
        Ok(data) => {
            println!(
                "Flate output (first 50 bytes): {:02x?}",
                &data[..data.len().min(50)]
            );
            println!(
                "Flate output as string: {:?}",
                String::from_utf8_lossy(data)
            );
        }
