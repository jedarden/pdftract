//! Quick test to verify basic extraction works on known-good fixtures.

use pdftract_core::options::ExtractionOptions;
use pdftract_core::sdk;
use std::path::{Path, PathBuf};

/// Workspace root, resolved relative to this crate so the test never depends
/// on an absolute checkout path.
fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("..").join("..")
}

#[test]
fn test_extract_tagged_pdf() {
    let path = workspace_root().join("tests/fixtures/tagged-suspects-false.pdf");
    let options = ExtractionOptions::default();

    let result = sdk::extract(&path, &options).unwrap();
    println!("Pages extracted: {}", result.pages.len());
    assert!(result.pages.len() > 0, "Should extract at least one page");
}

#[test]
fn test_extract_base_hello() {
    let path = workspace_root().join("tests/document_model/fixtures/base_hello.pdf");
    let options = ExtractionOptions::default();

    let result = sdk::extract(&path, &options).unwrap();
    println!("Pages extracted: {}", result.pages.len());
    assert!(result.pages.len() > 0, "Should extract at least one page");
}

#[test]
fn test_extract_conformance_fixture() {
    let path = workspace_root().join("tests/sdk-conformance/fixtures/scientific_paper/01.pdf");
    let options = ExtractionOptions::default();

    let result = sdk::extract(&path, &options).unwrap();
    println!("Pages extracted: {}", result.pages.len());
    assert!(result.pages.len() > 0, "Should extract at least one page");
}
