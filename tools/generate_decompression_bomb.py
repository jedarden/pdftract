...
Usage:
    python3 generate_decompression_bomb.py [--size MB] [--output PATH]

Options:
    --size MB         Decompressed size in MB (default: 10)
    --output PATH      Output PDF path (default: tests/fixtures/malformed/bomb-10k-2g.pdf)
    --help             Show this help message

Examples:
    python3 generate_decompression_bomb.py
    python3 generate_decompression_bomb.py --size 100 --output custom-bomb.pdf
"""
...
def generate_bomb(decompressed_size_mb, output_path):
    """Generate a decompression bomb PDF.

    Args:
        decompressed_size_mb: Target decompressed size in MB
        output_path: Where to write the PDF
    """
    # For simplicity and safety, we'll create a smaller but still dangerous bomb:
    # Target size MB of highly compressible data that fits in ~10KB compressed
    # This is still a 1000:1 compression ratio, sufficient for testing
    decompressed_size = decompressed_size_mb * 1024 * 1024
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate PDF decompression bomb test fixture for TH-01",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help='Decompressed size in MB (default: 10)'
    )
    parser.add_argument(
        '--output',
        default='tests/fixtures/malformed/bomb-10k-2g.pdf',
        help='Output PDF path (default: tests/fixtures/malformed/bomb-10k-2g.pdf)'
    )

    args = parser.parse_args()

    generate_bomb(args.size, args.output)
