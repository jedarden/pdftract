...
## Per-Release Gate (normative)

The smoke test is a **gated, repeatable step of every milestone release**, not
an ad-hoc check. It is wired into the release cascade as follows:

**When it runs.** The Argo release cascade (`pdftract-release-cascade` →
`pdftract-github-release`) publishes artifacts and opens the GitHub Release —
that publication is where the cascade ends. The smoke gate starts there: once
the archives are downloadable, the executor runs the checklist against the
published artifacts on each manual-smoke platform. The release stays
**provisional** (not marked latest, not announced) until the gate completes.

**What the repeatable procedure is.** Each release works from the committed
canonical fixture set and goldens in `tests/smoke/`:

1. Install the release archive for the triple under test.
2. Run `pdftract doctor` (Step 1 below) — 0 FAIL required.
3. Run `tests/smoke/diff_goldens.sh <installed-binary>` — extraction over the
   canonical corpus, byte-diffed against the committed goldens.
4. Record per-fixture results and the installed binary's SHA256 in the
   checklist.

The corpus is byte-stable across releases (`tests/smoke/corpus.manifest` pins
exact fixture bytes; see `tests/smoke/README.md` for the golden lifecycle and
the current bootstrap status). Goldens change only by deliberate
re-recording, which is what makes results comparable release-over-release.

**What artifact proves it.** The completed checklist — copied from
[platform-smoke-checklist-template.md](platform-smoke-checklist-template.md)
to `platform-smoke-<version>.md`, all three platform blocks filled in — is
**attached to the GitHub Release** for that version. The release notes
generated by the cascade carry a "Platform smoke verification" section that
links this runbook and stays unticked until the artifact is attached.

**Gate outcome.**

- **All blocks PASS** → release is final: mark latest and announce.
- **Any FAIL** → release remains provisional; fix and re-tag, or record an
  explicit release-lead waiver in the checklist.
- **Any BLOCKED step** (e.g. goldens not yet recorded for this corpus
  revision — `diff_goldens.sh` exit 3) → release remains provisional; the
  checklist names the tracking bead that unblocks it.

The quarterly KU-12 full pass (all steps below, including OCR/profile/cache)
remains unchanged and can reuse the same checklist artifact.
...
- **Bead:** `pdftract-653ah` (runbook integration); `pdftract-0b43c834` (per-release gate wiring)
- **Checklist template:** [platform-smoke-checklist-template.md](platform-smoke-checklist-template.md)
