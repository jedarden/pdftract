...
yields the committed file bit-for-bit. Re-verified at HEAD `669f3d3f`
(2026-09-14): regeneration still byte-identical, and the full invariant set
re-derived (see History).
...
- `crates/pdftract-core/tests/cmap_unmapped_glyphs.rs` — CMAP/ToUnicode skip
  behavior (8/8 as of HEAD `669f3f`); the 5 failures recorded on 2026-09-13
  were the documented missing `build/unmapped-glyph-names.json` baseline
  (notes/bf-2nob5-audit.md §3), resolved by committing the config at
  `crates/pdftract-core/build/unmapped-glyph-names.json` in `bea94b41`
...
- **Re-verified:** 2026-09-14 (bf-5oytjz attempt 2, HEAD `669f3d3f`) —
  regenerated byte-identical; /Differences completeness, /ToUnicode absence,
  CMap-stream absence and content-stream code resolution re-confirmed at byte
  level; font::encoding + font::cmap 49/49, no_mapping_fixture_structure 7/7,
  unmapped_glyph_names_config 4/4, cmap_unmapped_glyphs 8/8 (baseline WARN
  resolved by `bea94b41` committing the unmapped-glyph-names config)
